      ...
      <div className="px-4 py-6 space-y-8 animate-fade-in">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
            {services?.map((svc: any) => (
              ...
              >
                ...
                <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider font-semibold">Mulai Rp {svc.basePrice.toLocaleString('id-ID')}</p>
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          {loadingMechanics ? (
            ...
            <div className="space-y-3">
              {mechanics.slice(0, 5).map((m: any) => (
                <button 
                  key={m.id} 
                  onClick={() => setSelectedMechanic(m)}
                  className={`w-full p-4 flex items-center gap-4 text-left transition-all rounded-2xl border-2 ${
                    selectedMechanic?.id === m.id 
                      ? 'border-primary bg-primary/10 ring-1 ring-primary' 
                      : 'border-slate-800 bg-slate-800/50 hover:border-slate-700'
                  }`}
                >
                  <div className="w-14 h-14 rounded-2xl overflow-hidden bg-slate-700 flex-shrink-0 relative">
                    {m.user.avatarUrl ? (
            ...
            <div className="p-8 rounded-2xl bg-danger/5 border border-danger/10 text-center">
